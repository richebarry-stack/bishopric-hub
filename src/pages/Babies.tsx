    ...
    <div>
      ...
      {isLoading ? <p className="text-gray-400 text-sm">Loading...</p> : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {rows.map(b => (
            <div key={b.id} className="bg-white rounded-lg border border-gray-200 p-4 cursor-pointer hover:shadow-sm" onClick={() => setEditing(b)}>
              <div className="flex items-center justify-between mb-2">
                <h3 className="font-medium text-gray-900">{b.name}</h3>
                <div className="flex items-center gap-2">
                  <StatusBadge status={b.status} colors={COLORS} />
                  <button onClick={e => { e.stopPropagation(); remove(b.id); }} className="text-red-400 hover:text-red-600 text-xs">Del</button>
                </div>
              </div>
