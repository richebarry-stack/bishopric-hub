...
const ALL_CALENDARS: ScheduleCalendarName[] = ['Bishop', 'First Counselor', 'Second Counselor'];
...
// screens, but tall enough that an appointment's title text stays readable.
// Below this, the grid scrolls instead of cramming everything onto the screen.
...
interface FormState {
  ...
  title: string;
  notes: string;
  calendars: ScheduleCalendarName[];
...
export default function ScheduleCalendar({ availableCalendars, title, subtitle }: {
  availableCalendars: ScheduleCalendarName[];
  title: string;
  subtitle: string;
}) {
  ...
  const beginDrag = (
    ...
    const onMove = (e: PointerEvent) => {
      ...
      if (!el) return;
      const td = (el as HTMLElement).closest('[data-sc-si]') as HTMLElement | null;
      if (!td) return;
      const si = Number(td.dataset.scSi);
      const date = td.dataset.scDate;
      if (!date) return;
      if (d.type === 'move') {
  ...
  const handleSlotClick = (date: string, time: string) => {
    ...
    resetRepeatState();
    setEditing({ date, start_time: time, end_time: TIME_SLOTS[endIdx], title: '', notes: '', calendars: [activeTab] });
  };

  const openEditor = (entry: ScheduleEntry) => {
    setEditing({
      id: entry.id, date: entry.date.slice(0, 10), start_time: entry.start_time, end_time: entry.end_time,
      title: entry.title, notes: entry.notes || '', calendars: parseCalendars(entry.calendars),
  ...
  const handleSave = async () => {
    if (!editing || !editing.title.trim() || editing.calendars.length === 0) return;
    const data: Record<string, unknown> = {
      date: editing.date, start_time: editing.start_time, end_time: editing.end_time,
      title: editing.title, notes: editing.notes, calendars: stringifyCalendars(editing.calendars),
    };
    setSaving(true);
    try {
      if (editing.id) {
  ...
  // Saves this occurrence normally, then copies its title/notes/times/calendars (not date or
  ...
  const handleSaveToFutureSeries = async () => {
    ...
    try {
      ...
      const contentData = {
        start_time: editing.start_time, end_time: editing.end_time, title: editing.title, notes: editing.notes,
        calendars: stringifyCalendars(editing.calendars),
      };
  ...
  const handleQuickDelete = async (entry: ScheduleEntry) => {
  ...
  const toggleEditingCalendar = (cal: ScheduleCalendarName, checked: boolean) => {
    setEditing(prev => {
      if (!prev) return prev;
      const next = checked ? [...prev.calendars, cal] : prev.calendars.filter(c => c !== cal);
      ...
      {availableCalendars.length > 1 && (
        <div className="flex gap-1 mb-4 border-b border-gray-200">
          {availableCalendars.map(cal => (
            <button key={cal} onClick={() => setActiveTab(cal)}
              className={`px-4 py-2 text-sm font-medium border-b-2 -mb-px ${
                activeTab === cal ? 'border-blue-600 text-blue-700' : 'border-transparent text-gray-500 hover:text-gray-700'
              }`}>
              {cal}
            </button>
          ))}
                        ...
                        >
                          ...
                          {startingHere.map((entry, entryIdx) => {
                            ...
                            return (
                              <div
                                key={entry.id}
                                className={`group absolute text-white rounded px-1 py-0.5 overflow-hidden shadow-sm select-none
                                ...
                                onPointerDown={e => {
                                  ...
                                  const offsetSlots = Math.floor((e.clientY - rect.top) / rowHeight);
                              ...
                              >
                                <button
                                  ...
                                  className="absolute top-0 right-0 w-4 h-4 flex items-center justify-center
                                    text-white/70 hover:text-white hover:bg-black/20 rounded-bl opacity-0 group-hover:opacity-100 z-10"
                                  onPointerDown={e => e.stopPropagation()}
                                  onClick={e => { e.stopPropagation(); handleQuickDelete(entry); }}
                                >
                                  ×
                                </button>
                                <div className="font-medium truncate pr-3">{entry.title}</div>
                                {span > 2 && (
                          ...
                          {isPreviewHere && (() => {
                            ...
                            return (
                              <div
                                ...
                                style={{ top: 0, height: Math.max(1, span) * rowHeight - 1, lineHeight: '1.2', opacity: 0.8, zIndex: 20 }}
                              >
          ...
          <form onSubmit={e => { e.preventDefault(); handleSave(); }} className="space-y-3">
            ...
            <div>
              ...
              <div className="mt-1 flex flex-wrap gap-x-4 gap-y-1">
                {ALL_CALENDARS.map(cal => (
                  <Checkbox key={cal} label={cal} checked={editing.calendars.includes(cal)}
                    onChange={checked => toggleEditingCalendar(cal, checked)} />
                ))}
              </div>
            ...
            <div className="flex justify-between pt-2">
              ...
              <div className="flex gap-2">
                ...
                <button type="submit" disabled={saving || editing.calendars.length === 0} className="px-4 py-2 bg-blue-600 text-white rounded-md text-sm hover:bg-blue-700 disabled:opacity-50">
                  {saving ? 'Saving…' : 'Save'}
                </button>
              </div>
            </div>
